  ...
  <div>
    ...
    >
      ...
      <template v-slot:action>
        ...
        >
          <v-btn text @click="removeMessage(snackbar.key, true)" :color="snackbar.color">close</v-btn>
        </slot>
      </template>
    </v-snackbar>
...
export default {
  ...
  computed: {
    ...
    messages() {
      return this.$store.state.snackbar.messages;
    },
    timeout() {
    ...
    objects() {
      return this.$store.state.snackbar.objects;
    }
  },
  watch: {
    messages() {
      ...
      if (idx > -1) {
        ...
        let removeSnackbar = () => {
          ...
          if (fromComponent) {
            ...
            this.$store.commit('snackbar/setMessages', this.allMessages.filter((m, i) => i !== idx))
            this.$store.commit('snackbar/setObjects', this.objects.filter((m, i) => i !== idx))
          }
        }
